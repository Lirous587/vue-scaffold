...
const request = {
  get: async <T = any>(url: string, config?: any): Promise<SuccessResponse<T>> => {
    const response = await instance.get<SuccessResponse<T>>(url, config)
    return response.data
  },
  post: async <T = any>(url: string, data?: any, config?: any): Promise<SuccessResponse<T>> => {
    const response = await instance.post<SuccessResponse<T>>(url, data, config)
    return response.data
  },
  put: async <T = any>(url: string, data?: any, config?: any): Promise<SuccessResponse<T>> => {
    const response = await instance.put<SuccessResponse<T>>(url, data, config)
    return response.data
  },
  delete: async <T = any>(url: string, config?: any): Promise<SuccessResponse<T>> => {
